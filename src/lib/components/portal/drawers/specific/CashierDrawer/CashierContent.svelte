<script lang="ts">
  ...
  import { cashierDrawer } from "$lib/state/portal/cashier-drawer.state.svelte";
  ...
  // ============================================
  // Balance Sync on Drawer Open
  // ============================================

  $effect(() => {
    if (cashierDrawer.isOpen && user.principal && user.isAuthenticated) {
      userPortfolio.syncWithEntityStore(user.principal as any).catch((err) => {
        console.error('[CashierDrawer] Failed to sync balances:', err);
      });
    }
  });
