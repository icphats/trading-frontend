...
<div class="space-y-6">
  <div class="bg-[var(--background)] border border-[var(--border)] rounded-3xl p-4 sm:p-6 space-y-6 overflow-hidden">
    ...
    <div>
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
        <div>
          ...
          <p class="text-sm text-[color:var(--muted-foreground)] mt-1">Mint tokens to one or more addresses at creation
            <span class="text-xs">({tokenCreation.initialBalances.length.toLocaleString()} / 1,000 recipients)</span>
          </p>
        </div>
          ...
          <div class="flex flex-col sm:flex-row gap-3 sm:items-start">
            ...
            <div class="flex-1">
              <input
                ...
                class="w-full px-4 py-3 bg-[color:var(--background)] border rounded-[var(--radius-md)] text-sm form-input font-mono"
                class:border-red-500={row.principal && !tokenCreation.isValidPrincipal(row.principal)}
                class:border-[color:var(--border)]={!row.principal || tokenCreation.isValidPrincipal(row.principal)}
              />
              {#if row.principal && !tokenCreation.isValidPrincipal(row.principal)}
                <p class="text-xs text-red-500 mt-1">Invalid principal format</p>
              {/if}
            </div>
            ...
            <div class="w-full sm:w-64">
              <div class="relative">
                <input
                  ...
                  placeholder="1000000"
                  class="w-full px-4 py-3 pr-16 bg-[color:var(--background)] border border-[color:var(--muted-foreground)] rounded-[var(--radius-md)] text-sm form-input"
            ...
            <button
              ...
              class="self-end sm:self-auto p-3 text-[color:var(--muted-foreground)] hover:text-red-500 hover:bg-red-500/10 rounded-[var(--radius-md)] transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
    ...
    <button
      ...
      disabled={tokenCreation.atMaxBalances}
      class="mt-3 w-full px-4 py-3 border border-dashed border-[color:var(--border)] rounded-[var(--radius-md)] text-sm text-[color:var(--muted-foreground)] hover:border-[color:var(--primary)] hover:text-[color:var(--primary)] transition-colors disabled:opacity-30 disabled:cursor-not-allowed disabled:hover:border-[color:var(--border)] disabled:hover:text-[color:var(--muted-foreground)]"
    >
      <span class="flex items-center justify-center gap-2">
        <svg class="w-4 h-4" viewBox="0 0 16 16" fill="currentColor">
          <path d="M8 4a.5.5 0 01.5.5v3h3a.5.5 0 010 1h-3v3a.5.5 0 01-1 0v-3h-3a.5.5 0 010-1h3v-3A.5.5 0 018 4z" />
        </svg>
        {tokenCreation.atMaxBalances ? "Maximum recipients reached" : "Add Recipient"}
      </span>
      </button>
    </div>
    ...
    <div class="flex items-center justify-between gap-4 mt-6">
      <ButtonV2 variant="secondary" size="lg" onclick={onBack}>Back</ButtonV2>
      <ButtonV2 variant="primary" size="lg" onclick={onNext} disabled={!tokenCreation.step2Valid}>Next</ButtonV2>
    </div>
  </div>
</div>
